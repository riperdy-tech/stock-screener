...
function fmt(val: any, prefix = '', suffix = '', decimals = 2): string {
    ...
    // Auto scale large numbers to M/B
...
function formatFinancialData(d: any): string {
    const lines: string[] = [];
    ...
    const annuals: any[] = d.Annual_Income_Statement || [];
    if (annuals.length > 0) {
        lines.push(`── ANNUAL INCOME STATEMENT ──────────────`);
        annuals.forEach((row: any) => {
    ...
    const quarters: any[] = d.Quarterly_Income_Statement || [];
    if (quarters.length > 0) {
        lines.push(`── QUARTERLY INCOME STATEMENT ───────────`);
        quarters.forEach((row: any) => {
...
export async function POST(req: NextRequest) {
  try {
    const { ticker, customTemplate } = await req.json();
    ...
    try {
        ...
        console.warn("'py' command failed, falling back to 'python'", e);
        const { stdout } = await execPromise(`python "${scriptPath}" --ticker ${ticker.toUpperCase()}`);
        stdoutData = stdout;
    }
    
    let financialData;
    try {
      financialData = JSON.parse(stdoutData.trim());
      if (financialData.error) {
        return NextResponse.json({ error: financialData.error }, { status: 500 });
      }
    } catch (parseError) {
      console.error("Failed to parse JSON", stdoutData);
      return NextResponse.json({ error: 'Failed to parse financial data', details: stdoutData }, { status: 500 });
    }

    // Use friend's template if provided, else use default expert template
