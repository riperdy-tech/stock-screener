...
**Addendum — root-cause test (user challenge: "two-in-a-row doesn't fix WHY it flips"):** V5 variants derived membership from deterministic fields only (live MoS ≥15, ± entry-timing tier), discarding the sampled action/conviction entirely. **Result: worse than current** (V5: −1.43/−1.71% @25 bps vs V0 −0.20%, with ~150 trades and a 14.7-name avg set vs 8.8) — the LLM's judgment excludes ~6 names/day that pure numbers would buy, and those exclusions added value. So the sampled layer carries real selection signal expressed noisily, and deleting it is the wrong cure. **Verified root cause of the noise: re-analyses are amnesiac** — `run_rs2.py` contains no reference to any previous verdict; each weekly run regenerates the call from scratch at temperature 0.4, so borderline names land on different sides run-to-run. Fundamental fix (prospective-only, GPU A/B testable): **anchored re-analysis** — inject the prior verdict into the final stage, require a named material change (`changed_because`) to alter the call; two-in-a-row remains as downstream defense; large-move trigger ensures real events re-review immediately.

**Addendum — anchor A/B result (2026-07-21, `RS2 Local/ab_anchor_test.py`, 48 regenerations + 2 smokes):** with the six analysis stages FROZEN, regenerating only the final call produced flips vs the source verdict in **4% of plain reps (1/25 — MPWR, the known flipper)** and **0% of anchored reps (0/25)**; anchored reps all emitted `Changed-Because: none (maintained previous call)`. Interpretation — the diagnosis sharpens: **the final emission step is nearly deterministic given a fixed analysis; the wild 35% flip rate must therefore enter upstream**, in the weekly from-scratch regeneration of the stage analyses (S1–S6, esp. S5 conviction) and refreshed research over daily-moving data. The final-stage anchor remains the right mechanism — it sits at the convergence point and licenses the model to maintain its call against a wobbling upstream analysis — but its live-condition effect could not be measured with frozen stages. **Acceptance test moves prospective:** enable `--anchor` in the orchestrator (excluded for exit reviews), watch the live flip rate (tracked by `tools/verdict_history.py`) fall from 35% toward single digits over 1–2 weeks of cadence; instantly reversible by removing the flag. Companion in the same diff: the large-move re-review trigger (>12% move since last analysis forces a fresh review, so anchored persistence can never sit through a real event).
