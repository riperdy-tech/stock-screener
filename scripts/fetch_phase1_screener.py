...
def main():
    ...
    for idx, ticker in enumerate(tickers):
        ...
        try:
            ...
            if share_row:
                latest_shares, prev_shares = safe_get_yoy_first_two(bs, share_row)
                if latest_shares is not None and prev_shares is not None and prev_shares > 0:
                    share_growth = (latest_shares / prev_shares) - 1
                    if share_growth > 0.03: # >3%
                        continue
            ...
            anti_empire_flag = False
            if pd.notna(asset_growth) and pd.notna(ebitda_growth):
                anti_empire_flag = bool(asset_growth <= ebitda_growth)
            ...
            automated_roic_vel = "Pass" if roic > 0.15 else "Fail" # Base heuristic
            try:
