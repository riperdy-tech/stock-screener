...
> **Legibility pass (Aug 24) — these token values supersede any earlier ones.** The canvas was lifted off near-black and every secondary/tertiary grey was raised, because muted greys on `#131416` were unreadable on real screens. Micro-labels also changed size/weight/tracking (see Typography). Nothing else about the design moved.

Colors (extend `tailwind.config.js`; replace the current palette in `globals.css`):
- Page background: `#15171a` (body) / `#1c1e21` (app surface) / `#202225` (inset panels, transcript viewers, tooltips)
- Text primary: `#f2f0eb` · secondary: `#d3cfc5` · tertiary/faint: `#c3bfb5` · body-quote: `#e0ddd6`
- Secondary and tertiary are deliberately close together. The hierarchy is carried by **size, weight, and tracking**, not by fading text toward the background. Do not reintroduce a dimmer grey tier.
...
- Negative: `#e2917f` (red) — overvalued verdict + band viz, vetoes, SELL tags
- Factor-mix bars: value `#5a9b6d`, quality `#6b93c4`, momentum `#cfa14e`, low-vol `#9a83c2`, revisions `#c2798f`
- Chart series: EQUAL accent 2.5px · PLAN `#e0ddd6` · PLAN2 `#c2798f` · MINE `#cfa14e` (solid, 1.8px) · benchmarks dashed `4 4`: IWM `#908d86`, SPY `#6b93c4`, QQQ `#4f9e8f`, DRAM `#8f7fc0`, SOXL `#b56a4f`
- Borders/rules: `rgba(255,255,255,.22)` (header rule), `.18` (table header rule), `.24` (inactive chip border), `.14` (section rules), `.1` (row dividers) — all one step stronger than before so structure still reads against the lighter canvas
- Hover row: `background: rgba(255,255,255,.05)` · Bar/strip tracks: `rgba(255,255,255,.12)` · Links: `#a8b4d8`, hover `#c3cce6`
- Scrollbars on dark scroll areas: `scrollbar-width: thin; scrollbar-color: rgba(255,255,255,.22) #15171a`
...
- Micro-label style (used everywhere): mono, **11px, weight 600**, `letter-spacing: .04–.07em`, uppercase, color `#d3cfc5`. This replaces the old 8.5–10px/400/`.1–.14em` label. Tracking is roughly half what it was: at 11px/600 the wide tracking made labels harder to read, not more editorial. Column headers may wrap to two lines — that is expected and fine.
- **11px is the floor for every piece of text in the app** — labels, axis ticks, slider captions, sector sublines, footnotes, mobile. Nothing renders below it. Old 8.5/9/10px text all moved to 11px; non-mono small text keeps its own weight (400–500), only mono micro-labels go to 600.
- Chart axis labels: mono 11px/600 `#c3bfb5`, y-labels get a `background: #1c1e21` chip (with `padding-right: 4px`) so they stay readable where a series line passes behind them.
...
- Table rows: 12–13px vertical padding, `border-bottom: 1px solid rgba(255,255,255,.1)`
- Section starts: `border-top: 2px solid #f2f0eb` + uppercase 800 header
- Bars/band strips: 4–16px tall, track `rgba(255,255,255,.12)`, square

## Screens / Views

### 1. App shell (all screens)
Header: baseline flex row — "STOCKPEAK" 21px/800 `.06em` + "AI RESEARCH DESK" mono 9.5px muted; nav links 13px/600, 26px gap (Rankings / Track Record / Portfolio / Handbook), active = `#f2f0eb` + 2px accent bottom border. Status strip below: mono 10px muted, run timestamp left, standing record right ("AI PICKS +14.1% VS IWM +8.3% · APPEND-ONLY", return in green 600).
...
- Lens switcher: mono LENS label + chips (RS2 AI / Quant filter / Compare); active chip `background:#f2f0eb;color:#1c1e21;700`. Right: stance/sector filters + search.
- **RESEARCH NOW** table, grid `26 180 190 170 90 120 150 70 56px` gap 12: # (AI rank) · STOCK · DEPTH VERDICT (verdict 13/800 in verdict color + subline "every run above the price · buy") · IV BAND VS PRICE (**mini band strip**: 16px track `rgba(255,255,255,.12)`; band segment = verdict-color at .28 alpha with 1px verdict-color side borders; 1px median tick; 2px `#f2f0eb` price tick; subline mono "$158–166 · MED 162") · MEDIAN GAP (mono 13/600, green/muted/red) · SPREAD → SIZE (mono "4.9% → FULL"; FULL accent, HALF/QUARTER amber, watchlist "—" muted) · QUANT FILTER (muted "92.8 · #2 · rsrch") · PRICE · MCAP. Row click → detail.
...
- Left: mono kicker "RS2 LOCAL-LLM ANALYSIS · DEPTH RUN {date} · 3 SEEDED RUNS"; verdict 26px/800 in verdict color + "— every run values it above the price" 13.5px `#e0ddd6`; **band chart hero** (58px track: shaded band verdict-color .16 alpha with side borders, median 1px line, 7px square dots per run IV, 2px price tick full height; $low/$high labels above band ends; below-row labels "$251.29 TODAY" / "MED $256" / "RUN 3"); stat row of four (MEDIAN IV + gap vs price · RUN SPREAD + "runs agree tightly" · SIZE HINT in accent + tier rule · PLAUSIBLE RUNS n/3); band-direction explainer paragraph 11px `#c3bfb5`; thesis blockquote (2px accent left border, `rgba(255,255,255,.03)` bg, 13.5/1.6 `#e0ddd6`); evidence chip row (FACT PACK version, GPR, forensic flags, entry date).
...
- **DEPTH RUN TRANSCRIPTS** — full-width section under the grid, `1px .16` top rule: header "DEPTH RUN TRANSCRIPTS — 3 SAMPLE REPORTS ({TICKER}_{run id})" + HIDE/SHOW toggle; tab chips "Sample 1 · $256" (active: green border + 8% green bg) etc.; viewer: `#15171a`, 1px `.09` border, max-height 340px, overflow-y auto (themed scrollbar), mono 11.5px/1.7 `#e0ddd6`, **`white-space: pre-wrap; overflow-wrap: anywhere`** — raw output wraps, the viewer NEVER scrolls horizontally. Footnote: "RAW MODEL OUTPUT · SCROLLS · LONG LINES WRAP".
...
- **Chart (full width)**: GROWTH OF $10,000 + **9 toggle chips** (EQUAL/PLAN/PLAN2/MINE/IWM/SPY/QQQ/DRAM/SOXL — ●/○ mark; on: brighter border+text, EQUAL accent; off `#c3bfb5` with `rgba(255,255,255,.24)` border). SVG polylines from real daily NAV series; y gridlines at 4 levels with $ labels (HTML overlay, mono 11px/600 `#c3bfb5` on a `#1c1e21` chip), 5 date labels along the bottom (`white-space: nowrap`).
- **Hover**: crosshair (1px `rgba(255,255,255,.25)`) + tooltip (bg `#15171a`, 1px `.22` border, date header + per-visible-series label/color/$ value rows, flips near edges) tracking daily values under the cursor.
